...
namespace Xuld.Tools.DplBuilder {
    public class DplBuilder {
        ...
        void ScanJavascript(string path, string name) {
            ...
            for(; i < lines.Length; i++) {
                ...
                if(StartWith(c, "using")) {
                    c = CleanUsings(c, 5);
                    if(c != null)
                        TestNamespace(c, true);
                } else if(StartWith(c, "imports")) {
                    c = CleanUsings(c, 7);
                    if(c != null)
                        TestNamespace(c, false);
        ...
        static Dictionary<string, bool> _showed = new Dictionary<string, bool>();

        private void TestNamespace(string nameOrPath, bool isJs) {
            string name;
            string path;

            ResolveNamespace(nameOrPath, isJs, out path, out name);

            if(path != null && !_sources.ContainsKey(path) && !_showed.ContainsKey(name)) {
                Info("未引用（肯能导致问题）：" + nameOrPath);
                _showed[name] = false;
            }
        }
