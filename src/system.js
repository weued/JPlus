...
var JPlus = {
	 ...
	 * 启用控制台调试。
	 * 
	 * @config {Boolean} 如果不存在控制台，将自动调整为 false 。
	 */
	trace : true,
...
(function(window) {
	...
	// / #if !defined(SupportIE9) && !defined(SupportIE8) &&
	// !defined(SupportIE6)
	// / #define SupportIE6
	// / #endif
	...
	p = namespace(
			'JPlus.',
			{
				 ...
				 * @param {String}
				 *            dataType 类型。
				 * @return {Object} 值。 这个函数会在对象内生成一个 data 字段， 并生成一个
				 *         data.dataType 对象返回。 如果原先存在 data.dataType, 则直接返回。
				 * @example <code>
				 * var obj = {};
				...
				data : function(obj, dataType) {
					...
					assert.isObject(obj,
							"JPlus.data(obj, dataType): 参数 {obj} ~。");
				...
				getData : function(obj, dataType) {
					...
					assert.isObject(obj,
							"JPlus.getData(obj, dataType): 参数 {obj} ~。");
				...
				setData : function(obj, dataType, data) {
					...
					assert.isObject(obj,
							"JPlus.setData(obj, dataType): 参数 {obj} ~。");
				...
				cloneEvent : function(src, dest) {
					...
					assert.isObject(src,
							"JPlus.cloneEvent(src, dest): 参数 {src} ~。");
					assert.isObject(dest,
							"JPlus.cloneEvent(src, dest): 参数 {dest} ~。");

					// event 作为系统内部对象。事件的拷贝必须重新进行 on 绑定。
					var eventName = src.$data, event = eventName
							&& eventName.event;
						...
						for (eventName in event)
							...
							event[eventName].handlers
									.forEach(function(handler) {
										...
										p.IEvent.on.call(dest, eventName,
												handler[0],
												handler[1] === src ? dest
														: handler[1]);
									});
				...
				loadStyle : function(url) {
					...
					return document.getElementsByTagName("HEAD")[0]
							.appendChild(apply(document.createElement('link'),
									{
										href : url,
										rel : 'stylesheet',
										type : 'text/css'
				...
				loadText : function(url, callback) {
					...
					assert.notNull(url,
							"JPlus.loadText(url, callback): 参数 {url} ~。");
				 ...
				 *            ns 名字空间。
				 * @param {Boolean}
				 *            isStyle=false 是否为样式表。 有关名字空间的说明， 见
				 *            {@link namespace} 。
				 * @example <code>
				...
				using : function(ns, isStyle) {
					...
					ns = p.resolveNamespace(ns, isStyle);
					...
					each.call(doms, function(dom) {
						return !dom[src]
								|| dom[src].toLowerCase().indexOf(ns) === -1;
					})
				 ...
				 * 
				 * <p>
				 * 比如 system/dom/keys.js 文件， 名字空间是 System.Dom.Keys 名字空间用来快速表示资源。
				 * {@link using} 可以根据制定的名字空间载入相应的内容。
				 * </p>
				 * 
				 * <p>
				...
				checkStatusCode : function(statusCode) {
					...
					return (statusCode >= 200 && statusCode < 300)
							|| statusCode == 304 || statusCode == 1223;
				},
				...
				IEvent : {
					...
					on : function(type, listener, bind) {
						...
						assert
								.isFunction(listener,
										'IEvent.on(type, listener, bind): 参数 {listener} ~。');
						...
						if (!evt) {
							...
							d[type] = evt = function(e) {
								var listener = arguments.callee, handlers = listener.handlers
										.slice(0), i = -1, len = handlers.length;
					...
					un : function(type, listener) {
						...
						assert(
								!listener || Function.isFunction(listener),
								'IEvent.un(type, listener): 参数 {listener} 必须是函数或空参数。',
								listener);
					...
					trigger : function(type, e) {
						...
						return !evt
								|| !(evt = evt[type])
								|| ((eMgr = getMgr(me, type)).trigger ? eMgr
										.trigger(me, type, evt, e) : evt(e));
					...
					one : function(type, listener, bind) {
						...
						assert.isFunction(listener,
								'IEvent.one(type, listener): 参数 {listener} ~。');
			...
			Object,
			{
				...
				implement : function(members) {
					...
					assert(
							...
							"Class.implement(members): 无法扩展类，因为 {members} 或 this.prototype 为空。",
							members);
					// 复制到原型 。
				...
				implementIf : function(members) {
					...
					assert(
							members && this.prototype,
							"Class.implementIf(members): 无法扩展类，因为 {members} 或 this.prototype 为空。",
							members);
				 ...
				 * @return this
				 *         <p>
				 *         由于一个类的事件是按照 xType 属性存放的，拥有相同 xType 的类将有相同的事件，为了避免没有
				 *         xType 属性的类出现事件冲突， 这个方法会自动补全 xType 属性。
				 *         </p>
				 * 
				 * <p>
				 * 这个函数是实现自定义事件的关键。
				...
				addEvents : function(events) {
					...
					assert(
							!events || o.isObject(events),
							"Class.addEvents(events): 参数 {event} 必须是一个包含事件的对象。 如 {click: { add: ..., remove: ..., initEvent: ..., trigger: ... } ",
							events);
					...
					if (events) {
						...
						var xType = hasOwnProperty.call(ep, 'xType') ? ep.xType
								: (ep.xType = (p.id++).toString());
				 ...
				 * 
				 * 这显然是不正确的。所以你应该把 参数 quick 置为 false ， 这样，
				 * A创建实例的时候，会自动解除共享的引用成员。
				 * 
				...
				extend : function(members) {
					...
					// 生成子类 。
					var subClass = hasOwnProperty.call(
							members = members instanceof Function ? {
								constructor : members
	...
	apply(
			o,
			{
				...
				extend : (function() {
					...
					return function(dest, src) {
						if (src) {
							assert(
									dest != null,
									"Object.extend(dest, src): 参数 {dest} 不可为空。",
									dest);

							for ( var i = p.enumerables.length, value; i--;)
								if (hasOwnProperty.call(src,
										value = p.enumerables[i]))
				 ...
				 *            iterable 对象，不支持函数。
				 * @param {Function}
				 *            fn 对每个变量调用的函数。 {@param {Object} value 当前变量的值}
				 *            {@param {Number} key 当前变量的索引} {@param {Number}
				 *            index 当前变量的索引} {@param {Array} array 数组本身}
				...
				each : function(iterable, fn, bind) {
					...
					assert(
							!Function.isFunction(iterable),
							"Object.each(iterable, fn, bind): 参数 {iterable} 不能是函数。 ",
							iterable);
					assert(Function.isFunction(fn),
							"Object.each(iterable, fn, bind): 参数 {fn} 必须是函数。 ",
							fn);
				 ...
				 * @param {Function}
				 *            fn 对每个变量调用的函数。 {@param {Object} value 当前变量的值}
				 *            {@param {Number} key 当前变量的索引} {@param {Array}
				 *            array 数组本身} {@return {Boolean} 如果中止循环， 返回 false。}
				 * @param {Object}
				 *            bind=iterable 函数执行时的作用域。
				...
				update : function(iterable, fn, dest, args) {
					...
					o
							.each(
									iterable,
									Function.isFunction(fn) ? function(value,
											key) {
										...
										if (value !== undefined)
											...
											: function(value, key) {
												...
												if (value != undefined) {
													...
													assert(
															!args || dest[key],
															"Object.update(iterable, fn, dest, args): 试图把iterable[{key}][{fn}] 放到 dest[key][fn], 但  dest[key] 是一个空的成员。",
															key, fn);
				 ...
				 *            obj 类实例。
				 * @param {Object}
				 *            options 参数。 这个函数会分析对象，并试图找到一个 属性设置函数。 当设置对象 obj 的
				 *            属性 key 为 value: 发生了这些事: 检查，如果存在就调用:
				 *            obj.setKey(value) 否则， 检查，如果存在就调用: obj.key(value)
				 *            否则， 检查，如果存在就调用: obj.key.set(value) 否则，检查，如果存在就调用:
				 *            obj.set(value) 否则，执行 obj.key = value;
				 * 
				 * @example <code>
				 ...
				 * @param {Object}
				 *            obj 变量。
				 * @return {String} 所有可以返回的字符串： string number boolean undefined
				 *         null array function element class date regexp object。
				 * @example <code> 
				...
				type : function(obj) {
					...
					// 对象， 直接获取 xType 。
					return obj ? obj.xType || typeName : obj === null ? "null"
							: typeName;
	...
	applyIf(
			Array,
			{
				...
				create : function(iterable, startIndex) {
					...
					assert(
							!iterable
									|| toString.call(iterable) !== '[object HTMLCollection]'
									|| typeof iterable.length !== 'number',
							'Array.create(iterable, startIndex): 参数 {iterable} 不支持 DomCollection 。',
							iterable);
	...
	apply(Function, {
		 ...
		 *            fn 函数。
		 * @param {Object}
		 *            bind 位置。 注意，未来 Function.prototype.bind 是系统函数，
		 *            因此这个函数将在那个时候被 替换掉。
		 * @example <code>
	...
	apply(
			String,
			{
				...
				format : function(format, args) {
					...
					assert(!format || format.replace,
							'String.format(format, args): 参数 {format} 必须是字符串。',
							format);

					// 支持参数2为数组或对象的直接格式化。
					var toString = this;

					args = arguments.length === 2 && o.isObject(args) ? args
							: ap.slice.call(arguments, 1);
					...
					return (format || "")
							.replace(
									rFormat,
									function(match, name) {
										var start = match.charAt(1) == '{', end = match
												.charAt(match.length - 2) == '}';
										if (start || end)
				 ...
				 *            source 更新的函数或源。
				 * @param {Object}
				 *            [dest] 如果指明了， 则拷贝结果到这个目标。
				 * @param {Boolean}
				 *            copyIf=false 是否跳过本来存在的数据。
				 * @example <code>
				...
				map : function(str, src, dest, copyIf) {
					...
					assert(
							typeof str == 'string',
							'String.map(str, src, dest, copyIf): 参数 {str} 必须是字符串。',
							str);
				...
				ellipsis : function(value, len) {
					assert.isString(value,
							"String.ellipsis(value, len): 参数  {value} ~。");
					assert.isNumber(len,
							"String.ellipsis(value, len): 参数  {len} ~。");
					return value.length > len ? value.substr(0, len - 3)
							+ "..." : value;
				}
	...
	applyIf(
			navigator,
			(function(ua, isNonStandard) {

				// 检查信息
				var match = ua
						.match(/(IE|Firefox|Chrome|Safari|Opera|Navigator).((\d+)\.?[\d.]*)/i)
						|| [ "", "Other", 0, 0 ],
				...
				navigator["is" + browser] = navigator["is" + browser
						+ version[3]] = true;
				...
				return {
					...
					/**
					 * 获取一个值，该值指示当前浏览器是否支持标准事件。就目前浏览器状况， IE6，7 中 isQuirks = true
					 * 其它皆 false 。
					 * 
					 * @type Boolean 此处认为 IE6,7 是怪癖的。
					 */
					isQuirks : isNonStandard
							&& !o.isObject(document.constructor),
	...
	Object
			.implement({
				...
				base : function(methodName, args) {
					...
					assert(
							fn,
							"Object.prototype.base(methodName, args): 子类不存在 {methodName} 的属性或方法。",
							name);
					...
					assert(
							!me || me.prototype[methodName],
							"Object.prototype.base(methodName, args): 父类不存在 {methodName} 的属性或方法。",
							name);
					...
					assert
							.isFunction(fn,
									"Object.prototype.base(methodName, args): 父类的成员 {fn}不是一个函数。  ");
	...
	Array
			.implementIf({
				...
				insert : function(index, value) {
					assert
							.isNumber(index,
									"Array.prototype.insert(index, value): 参数 index ~。");
				...
				invoke : function(func, args) {
					assert(
							args && typeof args.length === 'number',
							"Array.prototype.invoke(func, args): 参数 {args} 必须是数组, 无法省略。",
							args);
					var r = [];
					ap.forEach
							.call(
									...
									function(value) {
										assert(
												value != null && value[func]
														&& value[func].apply,
												"Array.prototype.invoke(func, args): {value} 不包含函数 {func}。",
												value, func);
	...
	if (!p.rootPath) {
		try {
			...
			scripts = navigator.isQuirks ? scripts.getAttribute('src', 5)
					: scripts.src;
	 ...
	 * 
	 * @param {Function}
	 *            fn 遍历的函数。参数依次 value, index, array 。
	 * @param {Object}
	 *            bind 对象。
	...
	function each(fn, bind) {
		...
		assert(Function.isFunction(fn),
				"Array.prototype.each(fn, bind): 参数 {fn} 必须是一个函数。", fn);
	...
	function namespace(ns, obj) {
		...
		assert(ns && ns.split,
				"namespace(namespace, obj, value): 参数 {namespace} 不是合法的名字空间。",
				ns);
...
Object.extend(String, {
	...
	toUTF8 : function(s) {
		return s.replace(/[^\x00-\xff]/g, function(a, b) {
			return '\\u'
					+ ((b = a.charCodeAt()) < 16 ? '000' : b < 256 ? '00'
							: b < 4096 ? '0' : '') + b.toString(16);
		});
	},
	...
	fromUTF8 : function(s) {
		return s.replace(/\\u([0-9a-f]{3})([0-9a-f])/gi, function(a, b, c) {
			return String
					.fromCharCode((parseInt(b, 16) * 16 + parseInt(c, 16)))
		})
	}
...
function trace(obj, args) {
	...
	if (arguments.length == 0 || !JPlus.trace)
		return; // 关闭调试
...
Object
		.extendIf(
				trace,
				{
					 ...
					 * 
					 * @param {Object}
					 *            [obj] 要查看成员的对象。如果未提供这个对象，则显示全局的成员。
					 * @param {Boolean}
					 *            showPredefinedMembers=true 是否显示内置的成员。
					 */
					api : (function() {
						...
						definedClazz = 'String Date Array Number RegExp Function XMLHttpRequest Object'
								.split(' ').concat(nodeTypes),
						...
						APIInfo = Class({
							...
							getTypeName : function(obj, base, baseName, deep) {
								...
								if (deep-- > 0) {
									for ( var memberName in base) {
										try {
											if (base[memberName]
													&& isUpper(memberName, 0)) {
												memberName = this.getTypeName(
														obj, base[memberName],
														baseName + memberName
																+ ".", deep);
												if (memberName)
							...
							getBaseClassDescription : function(obj) {
								if (obj && obj.base) {
									var extObj = this.getTypeName(obj.base,
											window, "", 3);
									return " 类"
											+ (extObj
													&& extObj != "JPlus.Object" ? "(继承于 "
													+ extObj + " 类)"
													: "");
								}
							...
							constructor : function(obj, showPredefinedMembers) {
								...
								this.isClass = obj === Function
										|| (obj.prototype && obj.prototype.constructor !== Function);

								// 如果是普通的变量。获取其所在的原型的成员。
								if (!this.isClass && obj.constructor !== Object) {
									this.prefix = this
											.getPrefix(obj.constructor);

									if (!this.prefix) {
										var nodeType = obj.replaceChild ? obj.nodeType
												: obj.setInterval
														&& obj.clearTimeout ? 0
														: null;
										if (nodeType) {
									...
									if (this.prefix) {
										this.title = this.prefix
												+ this
														.getBaseClassDescription(obj.constructor)
												+ "的实例成员: ";
										this.prefix += '.prototype.';
									}

									if ([ Number, String, Boolean ]
											.indexOf(obj.constructor) === -1) {
								...
								if (this.isClass) {
									...
									this.addStaticMembers(obj.prototype, 1,
											true);
									delete this.members.prototype;
									if (this.showPredefinedMembers) {
										this.addPredefinedNonStaticMembers(obj,
												obj.prototype, true);
										this.addPredefinedMembers(obj, obj,
												predefinedStatic);
									}
									...
									if (this.showPredefinedMembers
											&& obj.constructor) {
										this.addPredefinedNonStaticMembers(
												obj.constructor, obj);
									}
							...
							addStaticMembers : function(obj, nonStatic) {
								for ( var memberName in obj) {
									try {
										this.addMember(obj, memberName, 1,
												nonStatic);
									} catch (e) {
									}
								}
							...
							addPredefinedMembers : function(clazz, obj,
									staticOrNonStatic, nonStatic) {
								for ( var type in staticOrNonStatic) {
									if (clazz === window[type]) {
										staticOrNonStatic[type].forEach(
												function(memberName) {
													this.addMember(obj,
															memberName, 5,
															nonStatic);
							...
							addPredefinedNonStaticMembers : function(clazz,
									obj, nonStatic) {

								if (clazz !== Object) {

									predefinedNonStatic.Object
											.forEach(
													function(memberName) {
								...
								this.addPredefinedMembers(clazz, obj,
										predefinedNonStatic, nonStatic);

							},

							addMember : function(base, memberName, type,
									nonStatic) {

								var hasOwnProperty = Object.prototype.hasOwnProperty, owner = hasOwnProperty
										.call(base, memberName), prefix, extInfo = '';

								nonStatic = nonStatic ? 'prototype.' : '';

								// 如果 base 不存在 memberName 的成员，则尝试在父类查找。
								if (owner) {
									prefix = this.orignalPrefix
											|| (this.prefix + nonStatic);
									type--; // 自己的成员置顶。
								} else {
									...
									this.baseClasses
											.each(
													function(baseClass, i) {
														if (baseClass.prototype[memberName] === base[memberName]
																&& hasOwnProperty
																		.call(
																				baseClass.prototype,
																				memberName)) {
															prefix = this.baseClassNames[i]
																	+ ".prototype.";
								...
								this.sortInfo[this.members[memberName] = (type >= 4 ? '[内置]'
										: '')
										+ prefix
										+ getDescription(base, memberName)
										+ extInfo] = type + memberName;
							...
							copyTo : function(value) {
								...
								if (value.length) {
									...
									value.sort(function(a, b) {
										return sortInfo[a] < sortInfo[b] ? -1
												: 1;
									});
						...
						function getMemberType(obj, name) {
							...
							if (obj.prototype && obj.prototype.constructor)
								return !isEmptyObject(obj.prototype)
										|| isUpper(name, 0) ? '类' : '函数';

							// 最后判断对象。
							if (Object.isObject(obj))
								return name.charAt(0) === 'I'
										&& isUpper(name, 1) ? '接口' : '对象';
						...
						return function(obj, showPredefinedMembers) {
							...
							if (arguments.length === 0) {
								for ( var i = 0; i < 7; i++) {
									r.push(getDescription(window,
											definedClazz[i]));
								}

								for ( var name in JPlus)
									if (window[name]
											&& (isUpper(name, 0) || window[name] === JPlus[name]))
								...
								new APIInfo(obj, showPredefinedMembers)
										.copyTo(r);
							} else {
								r.push('无法对 '
					...
					inspect : function(obj, deep) {
							...
							if (deep == 0 && obj.prototype
								...
								return String
										.format(
												...
												obj.prototype.xType,
												(obj.prototype.base
														&& obj.prototype.base.xType || "Object"),
												trace.inspect(obj.prototype,
														deep + 1));
							}

							// 函数
							return deep == 0 ? String.fromUTF8(obj.toString())
									: "function ()";
						...
						case "object":
							...
							if (Array.isArray(obj)) {
								return "["
										+ Object.update(obj, trace.inspect, [])
												.join(", ") + "]";
								...
								if (obj.nodeType) {
									...
									return '[Node name=' + obj.nodeName
											+ 'value=' + obj.nodeValue + ']';
								}
								var r = "{\r\n", i;
								for (i in obj)
									r += "\t" + i + " = "
											+ trace.inspect(obj[i], deep + 1)
											+ "\r\n";
								r += "}";
								return r;
							}
					...
					log : function() {
						if (JPlus.trace) {
							if (window.console && console.log
								...
								trace(Object.update(arguments, trace.inspect,
										[]).join(" "));
							}
						}
					},
	...
	if (!bValue && JPlus.debug) {
		...
		if (val.length > 2) {
			...
			msg = msg.replace(/\{([\w\.\(\)]*?)\}/g, function(s, x) {
				return val.length <= i ? s : x + " = "
						+ String.ellipsis(trace.inspect(val[i++]), 200);
			});
		} else {
			msg = msg || "断言失败";
		}
...
(function() {
	...
	Object.extend(assert, {
		...
		isObject : function(value, msg) {
			return assertInternal(Object.isObject(value)
					|| Function.isFunction(value) || value.nodeType, msg,
					value, "必须是引用的对象", arguments);
		},
		...
		isNumber : function(value, msg) {
			return assertInternal(typeof value == 'number'
					|| value instanceof Number, msg, value, "必须是数字");
		},
		...
		isNode : function(value, msg) {
			return assertInternal(value && value.nodeType, msg, value,
					"必须是 DOM 节点");
		},
		...
		isElement : function(value, msg) {
			return assertInternal(value && value.style, msg, value,
					"必须是 Element 对象");
		},
		...
		isString : function(value, msg) {
			return assertInternal(typeof value == 'string'
					|| value instanceof String, msg, value, "必须是字符串");
		},
		...
		isDate : function(value, msg) {
			return assertInternal(Object.type(value) == 'date'
					|| value instanceof Date, msg, value, "必须是日期");
		},
		...
		isRegExp : function(value, msg) {
			return assertInternal(Object.type(value) == 'regexp'
					|| value instanceof RegExp, msg, value, "必须是正则表达式");
		},
		...
		between : function(value, min, max, msg) {
			return assertInternal(value >= min && !(value >= max), msg, value,
					"超出索引, 它必须在 [" + min + ", "
							+ (max === undefined ? "+∞" : max) + ") 间");
		},
