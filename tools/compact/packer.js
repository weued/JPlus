...
var Colorizer = RegGrp.extend({
	...
	TAB : /\t/g,
	TABS : /\n([\t \xa0]+)/g,
	init : function() {forEach(this.patterns, function(c, d, e) {
			if(instanceOf(c, Array)) {
				e[d] = reduce(c, function(a, b) {a.add(e[b]);
					return a
				}, new RegGrp)
			}
		});
...
with(base2.code.Colorizer.javascript) {add("\\b(" + (base2.exports+",base,base2,merge,union,implement,Array2,Date2,String2").match(/[^\s,]+/g).join("|") + ")\\b", '<span class="base2">$0</span>');
...
new base2.JSB.RuleList({
	...
	"#message" : {
		...
		update : function(message) {
			...
			this.write(( message ? message + ", " : "") + format("压缩率: %1=%2", calc, 1 - ratio));
		},
